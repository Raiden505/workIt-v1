...
begin
  ...
  select ij.id, ij.client_id, ij.category_id, ij.title, ij.description, ij.budget, ij.status, ij.created_at
  from inserted_job as ij;
end;
$$;
...
begin
  ...
  select
    ...
    concat_ws(' ', p.first_name, p.last_name) as client_name,
...
begin
  ...
  if v_view = 'freelancer' then
    ...
    select
      ...
      c.end_date,
      j.title as job_title,
      concat_ws(' ', fp.first_name, fp.last_name) as freelancer_name,
